...
function get_tree() {
    ...
    $var = "<ul id=\"arbre\">\n\t\t<li>\n\t\t\t <span>&nbsp;</span> <a href=\"".$url->linkToObj('/').'" title="Revenir à la racine"><img src="'.REAL_ROOT_URL.DIR_TEMPLATE.'/img/home.png" width="32" height="32" border="0" align="middle" alt="Revenir à la racine" /></a>'."</li>\n";
    ...
    if ($tab) {
        ...
        foreach ($tab as $occ) {
            ...
            $var .= "\t\t\t <span>&nbsp;</span>".($current ? '<strong>' : null).
                    "<a href=\"".$url->linkToObj($occ->file).'"><img src="'.$occ->icon.'" class="icon" alt="" /> '.view_obj($tst[$num - 1])."</a>".
                    ($current ? '</strong>' : null).
                    "\n";
 ...
 */
function get_css() {
    global $page_styles, $conf;
    $out = null;
    $size = strlen($conf['style']);
    foreach($page_styles as $occ) {
        $out .= '<link rel="'.($conf['style'] == substr($occ['href'], strlen($occ['href']) - $size, strlen($occ['href'])) ? 'stylesheet' : 'alternate stylesheet').
...
function get_css_plugin() {
    global $page_styles_plugin, $conf;
    $out = null;

    if ($page_styles_plugin) {
        foreach($page_styles_plugin as $occ) {
            $out .= '<style type="'.$occ['type'].'">'."\n";
            // media="'.$occ['media'].'">';
            $out .= '@import "'.system::getHost().REAL_ROOT_URL.$occ['href'].'";'."\n";
            $out .= "</style>\n";
        }
/*
            $out .= '<link rel="stylesheet" type="'.$occ['type'].'" media="screen,projection" title="Standard" href="'.system::getHost().REAL_ROOT_URL.$occ['href'].'" />';
