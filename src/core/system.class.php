...
/*  Important !
 *  Catch timeout error or too much memory used error
 */
...
class system
{
    ...
    function getHost() {
        ...
        return $scheme.'://'.htmlspecialchars($_SERVER['HTTP_HOST']).$port;
    }
     ...
     */
    function osIsWin() {
        return (substr(system::getOS(), 0, 3) == 'win');
    }
    ...
    function timeOut() {
        if (!defined('EXIT')) {
            system::end('Fatal error : Time out !');
        }
    }
     ...
     */
    function log($type, $msg, $context = 'global') {
        global $cuser, $obj;
        $cobj = $obj->getCurrentObj();
        log::add($type, $msg, $context, $cuser->name, ($cobj) ? $cobj->file : null);
    }

    /**
     *  Test if variable is an error
     *  @param  mixed   $var    Var to test
     */
    function isError($var) {
        return (get_class($var) == 'tError');
    }

    /**
     *  Stop script execution immeditately
     *  @param  string  $msg    Message
     */
    function end($msg = null) {

        $basket = basket::getInstance();    // ToDo: mettre ça autre part
        $basket->save();
