...
class acl {
    ...
    function test($right) {
        ...
        if ($right == AUTHENTICATED_ID && $cuser->type == USR_TYPE_USER) {
            return true;
        }

        if ($right == ANONYMOUS_ID && $cuser->id == ANONYMOUS_ID) {
            return true;
        }
    ...
    function compare($id) {
        ...
        if ($id == ADMINISTRATOR_ONLY && $cuser->type == USR_TYPE_ADMIN) {
            return true;
        }

        if ($id == AUTHENTICATED_ID &&
            (   $cuser->type == USR_TYPE_USER ||
                $cuser->type == USR_TYPE_SUPERVISOR ||
                $cuser->type == USR_TYPE_ADMIN)) {
            return true;
        }

        if ($id == ANONYMOUS_ID && $cuser->id == ANONYMOUS_ID) {
            return true;
        }
