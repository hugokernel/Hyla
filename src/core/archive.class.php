...
class archive {
    ...
    function listContent($file) {
        ...
        include(HYLA_ROOT_PATH.'src/lib/pclerror.lib.php');
        include(HYLA_ROOT_PATH.'src/lib/pcltrace.lib.php');

        $type = archive::getType($file);
        switch ($type) {
            case 'zip':
                include(HYLA_ROOT_PATH.'src/lib/pclzip.lib.php');
                $zip = new PclZip($file);
                $ret = $zip->listContent();
                break;
            ...
            case 'tar.gz':
            case 'tgz':
                include(HYLA_ROOT_PATH.'src/lib/pcltar.lib.php');
                $ret = PclTarList($file);
                break;
        }
