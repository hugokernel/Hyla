...
class conf
{
     ...
     */
    /*
    function create($name, $value, $param) {
        ...
        // Get type
        $type = conf::getType($param);

        $usr_id = null;
        $plugin_context = null;
        if ($type == CONF_TYPE_USER) {
            $usr_id = $param;
        } else if ($type == CONF_TYPE_PLUGIN) {
            $plugin_context = $param;
        }

        $sql = "INSERT INTO {$this->_conf_table}
                (conf_type, conf_name, conf_usr_id, conf_plugin_context, conf_content_type, conf_content)
                VALUES
                ('$type', '$name', '$usr_id', '$plugin_context', '".conf::getContentType($value)."', '$value');";
        if ($var = $this->bdd->execQuery($sql)) {
            $ret = true;
        }
    ...
    */
    ...
    function get($name, $param = null) {
        return $this->_get($name, conf::getType($param), $param);
    }
    ...
    function set($tab, $param = null) {
        ...
        return $this->_set(conf::getType($param), $key, $value, $param);
    }
    ...
    function getType($type) {
        if ($type) {
            return (is_numeric($type)) ? CONF_TYPE_USER : CONF_TYPE_PLUGIN;
        } else {
            return CONF_TYPE_CORE;
        }
    }
