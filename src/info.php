...
switch ($cobj->type) {
	...
	case TYPE_ARCHIVED:
		...
		if (!file_exists($filepath)) {
			...
			if (function_exists('md5_file')) {
				$tpl->set_var(array(
						'MD5'				=>	(url::getQueryAff(2) == 'md5') ? ' : '.md5_file($filepath) : null,
						'URL_MD5_CALCULATE'	=>	url::getCurrentObj(array('info', 'md5')),
						));
	...
	case TYPE_FILE:
		...
		if (function_exists('md5_file')) {
			$tpl->set_var(array(
					'MD5'				=>	(url::getQueryAff(2) == 'md5') ? ' : '.md5_file($cobj->realpath) : null,
					'URL_MD5_CALCULATE'	=>	url::getCurrentObj(array('info', 'md5')),
					));
			$tpl->parse('Hdlaff_md5', 'aff_md5', true);
		}
