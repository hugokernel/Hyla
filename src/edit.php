...
if (acl::ok(AC_EDIT_DESCRIPTION)) {
	$tpl->set_var(array(
			'FORM_EDIT_SETDESCRIPTION'	=>	$url->linkToCurrentObj('edit', 'setdescription'),
...
if ($cobj->type == TYPE_DIR) {
	...
	if (acl::ok(AC_EDIT_ICON) && $cobj->file != '/') {
		...
		// L'icone par défaut
		if (empty($cobj->icon) || substr($cobj->icon, 0, strlen(REAL_ROOT_URL.DIR_IMG_PERSO)) != REAL_ROOT_URL.DIR_IMG_PERSO) {
		...
		if ($hdl) {
			while (false !== ($occ = $hdl->read())) {
				...
				if (REAL_ROOT_URL.DIR_IMG_PERSO.$occ == $cobj->icon) {
		...
		$tpl->set_var('FORM_EDIT_SETIMAGE', $url->linkToCurrentObj('edit', 'seticon'));
	...
	if (acl::ok(AC_EDIT_PLUGIN)) {
		$tab = plugin_obj::getDirPlugins();

		$tpl->set_var(array(
				'PLUGIN_DEFAULT_CHECKED'	=>	(!$cobj->info->plugin) ? 'value="default" checked="checked"' : 'value="default"',
				'FORM_EDIT_SETPLUGIN'		=>	$url->linkToCurrentObj('edit', 'setplugin'),
				'PLUGIN_DEFAULT_DIR'		=>	$conf['plugin_default_dir'],
				));
	...
	if (acl::ok(ADMINISTRATOR_ONLY)) {
		...
		switch ($url->getParam('aff', 3)) {
			...
			case 'edit':
				...
				if (isset($_POST['rgt_user'])) {
					$user = $_POST['rgt_user'];
				} else if ($url->getParam('aff', 4)) {
					$user = $url->getParam('aff', 4);
                }
				...
				if ($tab) {
					foreach ($tab as $key => $val) {
						...
						$tpl->set_var(array(
								...
								'URL_EDIT_RIGHTS'	=>	$url->linkToCurrentObj(array('edit', 'rights', 'edit', $key)),
								'URL_DEL_RIGHTS'	=>	$url->linkToCurrentObj(array('edit', 'rights', 'del', $key)),
								));
					...
					if ($last_obj) {
						$tpl->set_var(array(
								'URL_EDIT_RIGHTS_LAST'	=>	$url->linkToObj($last_obj, array('edit', 'rights')),
		...
		$tpl->set_var(array(
				'URL_RIGHTS'		=>	$url->linkToCurrentObj(array('edit', 'rights')),
				'URL_ADD_RIGHTS'	=>	$url->linkToCurrentObj(array('edit', 'rights', 'add')),
				'URL_ADMIN_RIGHTS'	=>	$url->linkToPage(array('admin', 'rights')),
				'FORM_EDIT_RIGHTS'	=>	$url->linkToCurrentObj(array('edit', 'rights', 'edit'), 'editrights'),
				'FORM_DEL_RIGHTS'	=>	$url->linkToCurrentObj(array('edit', 'rights'), 'delrights'),
				'FORM_ADD_RIGHTS'	=>	$url->linkToCurrentObj(array('edit', 'rights'), 'addrights'),
				));
