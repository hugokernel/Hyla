...
class plugin_obj_zenphoto extends plugin_obj { // implements _plugin {
	...
	function aff($paff) {
		...
		$img_width = $this->getConfVar('img_width', THUMB_SIZE_X);
		$img_height = $this->getConfVar('img_height', $img_width);
		...
		if ($tab) {
			...
			for ($i = 0, $last = null, $last_type = null; $i < $size; $i++) {			
				...
				$this->tpl->set_var(array(
						...
						'OBJECT_MINI'		=>	url::getObj($tab[$i]->file, array('mini', $img_width, $img_height)),
