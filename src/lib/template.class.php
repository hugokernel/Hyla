 ...
 * loadfile would try to load a file if the varval had been set to null (rha)
...
class Template
{
	...
	function Template($root = '.', $unknowns = 'remove') {
		...
		$this->set_root($root);
		$this->set_unknowns($this->unknowns);
		
		// C'est Achement pratique
		$this->set_var('FOLDER_TEMPLATE', ROOT_URL.'/'.$root);
	}
...
//		$root = '/var/www/list'.$root.'/';

		// Hugo, le 09/09/2002
		if (!is_dir($root))
		{
			// Si le répertoire n'existe pas dans le tpl, on prend celui de default
			$pos_start = strpos($root, '/');
			$pos_stop = strpos(substr($root, $pos_start + 1, strlen($root)), '/');
			$name_tpl = substr($root, $pos_start + 1, $pos_stop);
			if ($name_tpl != 'default')
				$root = substr_replace($root, 'default', $pos_start + 1, $pos_stop);
			
			// Si il existe toujours pas !
			if (!is_dir($root)) {
	 ...
	 */
	function set_file($varname, $filename = null) {
		if (!is_array($varname)) {
		  if ($filename == null) {
	...
	/* public: set_block(string $parent, string $varname, string $name = null)
	 * extract the template $varname from $parent,
	 * place variable {$name} instead.
	 */
	function set_block($parent, $varname, $name = null) {
		...
		else
		{
		  ...
		  {    
		    $str = $this->get_var($parent);
		    $reg = "/<!--\s+BEGIN $varnamet\s+-->(.*)\s*<!--\s+END $varnamet\s+-->/sm";
		    preg_match_all($reg, $str, $m);    
		    $str = preg_replace($reg, "{" . "$name}", $str);
	 ...
	 */
	function set_var($varname, $value = null) 
	{
		if (!is_array($varname))
		{
			if (!empty($varname))
	 ...
	 * Retourne les variables contenues dans la section en question
	...
	// Retourne les contenu d'une section sans les variables
	 ...
	 */
	function filename($_filename)
	{
		// Hugo, le 09/09/2002
		if (substr($_filename, 0, 1) != "/")
			$filename = $this->root.'/'.$_filename;

//		$filename = '/var/www/list'.$filename;

		if (!file_exists($filename)) {

			// Si le fichier n'existe pas dans le tpl, on prend celui de default
			$pos_start = strpos($filename, '/');
			$pos_stop = strpos(substr($filename, $pos_start + 1, strlen($filename)), '/');
			$name_tpl = substr($filename, $pos_start + 1, $pos_stop);
			if ($name_tpl != 'default')
				$filename = substr_replace($filename, 'default', $pos_start + 1, $pos_stop);
	...
	*/
	...
	function loadfile($varname) {
		...
		$str = implode(null, @file($filename));
		if (empty($str)) {
	...
	function haltmsg($msg) {
		error::log(__FILE__, __LINE__, "<b>Template Error:</b> $msg");
	}
