...
class obj extends acl {
	...
	function getInfo($name, $data = true, $pnext = true) {
		...
		if ($dir && ($rights & AC_VIEW)) {
			...
			// L'objet est un dossier
				...
				$obj->type = TYPE_DIR;
				$obj->path = $dir;
				$obj->name = (basename($name) == null) ? '/' : basename($this->_folder_root.$name);		// ToDo: est-ce bien d'utiliser basename en lieu et place de getRealFile ?
				$obj->file = $obj->path;
				$obj->realpath = $this->_folder_root.$obj->file;
				...
				if ($data) {
					...
					for ($i = 0; $res = $this->_bdd->nextTuple($var); $i++) {
						if (!$i) {
							...
							$obj->info->description = $res['obj_description'];
							$obj->info->dcount = (int)$res['obj_dcount'];
							$obj->info->plugin = strtolower($res['obj_plugin'] ? $res['obj_plugin'] : null);	// DIR_DEFAULT_PLUGIN); -> On met null, c'est mieux pour le par défaut !

							$obj->icon = $res['obj_icon'];
						}
				...
				if (!$obj->icon) {
					...
					$obj->icon = (file_exists($icon)) ? $icon : DIR_PLUGINS_OBJ.strtolower($conf['dir_default_plugin']).'/'.DIR_ICON;
				}
			}
		}
					...
					if (isset($a[$tab_out[$i]->file]['id'])) {
						...
						if ($tab_out[$i]->type == TYPE_DIR) {
							if ($a[$tab_out[$i]->file]['icon'])
								...
								$tab_out[$i]->icon = (($tab_out[$i]->info->plugin) ? (file_exists($icon) ? $icon : get_icon('.')) : DIR_PLUGINS_OBJ.$conf['dir_default_plugin'].'/'.DIR_ICON);
							}
						}	
						$tab_out[$i]->info->nbr_comment = isset($a[$tab_out[$i]->file]['nbr_comment']) ? $a[$tab_out[$i]->file]['nbr_comment'] : null;
					} else {
						if ($tab_out[$i]->type == TYPE_DIR) {
							$tab_out[$i]->icon = DIR_PLUGINS_OBJ.($tab_out[$i]->info->plugin ? $tab_out[$i]->info->plugin.'/' : '').$conf['dir_default_plugin'].'/'.DIR_ICON;
						}
					}
					$i++;
				}
	...
	function acceptAnonFile($file, $root) {
		...
		$sql = "SELECT	obj0.obj_file as destination, obj1.obj_file as name
				FROM	{$this->_object_table} obj0
				INNER	JOIN	{$this->_object_table} obj1 ON obj0.obj_id = obj1.obj_id_ref
				WHERE	obj1.obj_file = '".obj::format($file)."'";
	...
	function setPlugin($plugin) {
		...
		if ($plugin != $this->_current_obj->info->plugin && ($plugin == null || plugins::isValid($plugin))) {
			$id = $this->getId($this->_current_obj->file);
	...
	function setIcon($icon) {
		...
		if ($icon != $this->_current_obj->icon) {
			...
			if (!$icon) {
				...
				$icon = (file_exists($icon)) ? $icon : DIR_PLUGINS_OBJ.strtolower($conf['dir_default_plugin']).'/'.DIR_ICON;
			}
		}
		return $icon;
	}
	...
	function move($copy, $destination, $base = null) {
		...
		if (is_file($base.$copy)) {
//			$destination = $destination.basename($copy);
	...
	function syncBdd() {
		...
		for ($y = 0; $res = $this->_bdd->nextTuple($var); $y++) {
			...
			if ($res['flag'] == FLAG_ANON) {
				$file = DIR_ROOT.DIR_ANON.'/'.basename($res['object']);
			} else
	...
	function _sort_folder_first($a, $b) {
		...
		if (!$ret && $this->_tri & SORT_FOLDER_FIRST) {
			if (is_dir($a->realpath) && is_dir($b->realpath)) {
				$ret = strcasecmp($a->realpath, $b->realpath);
