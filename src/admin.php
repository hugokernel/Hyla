...
switch (url::getQueryAff(2)) {
	...
	case 'users':
		...
		$param = url::getQueryAff(3);

		switch ($param) {

			case 'add':
				if (url::getQueryAff(4) == 'save') {
			...
			case 'del':
				...
				if (url::getQueryAff(4) != $cuser->id) {
					if (!in_array((int)url::getQueryAff(4), array(ANY_ID, AUTHENTICATED_ID, ANONYMOUS_ID), true))
						$usr->delUser(url::getQueryAff(4));
				}
				$param = null;
				break;
			...
			case 'savetype':
				...
				if (url::getQueryAff(4) == $cuser->id) {
					...
					$usr->setType(url::getQueryAff(4), $tab_type[$_POST['ad_type']]);

			case 'savepassword':
				// Pas propre ça !
				if (url::getQueryAff(3) == 'savepassword') {
					...
					} else if (url::getQueryAff(4) != ANONYMOUS_ID && isset($_POST['ad_password']) && !empty($_POST['ad_password'])) {
						$usr->setPassword(url::getQueryAff(4), $_POST['ad_password']);
						$msg = view_status(__('Password changed !'));
					}
				}

			case 'edit':
				$tab = $usr->getUser(url::getQueryAff(4));
				if ($tab && !($tab->id == ANY_ID || $tab->id == ANONYMOUS_ID || $tab->id == AUTHENTICATED_ID)) {
					...
					$tpl->set_var(array(
							...
							'FORM_USER_EDIT_TYPE'		=>	url::getPage(array('admin', 'users', 'savetype', url::getQueryAff(4))),
							'FORM_USER_EDIT_PASSWORD'	=>	url::getPage(array('admin', 'users', 'savepassword', url::getQueryAff(4))),
							'MSG'						=>	$msg,
							));
		...
		$tpl->set_var(array(
				...
				'ADMIN_USER_LIST'	=>	url::getPage(array('admin', 'users')),
				));
	...
	case 'groups':
		...
		switch (url::getQueryAff(3)) {
			...
			case 'add':
				...
				$tpl->set_var(array(
						'FORM_GROUP_SAVE'	=>	url::getPage(array('admin', 'groups', 'add', 'save')),
						'ERROR'				=>	$msg_error,
						));
			...
			case 'edit':
				...
				$ginfo = $usr->getUser(url::getQueryAff(5));

				if ($ginfo && $ginfo->type == USR_TYPE_GRP) {

					if (url::getQueryAff(4) == 'del') {
						if (isset($_POST['ad_del_users'])) {
							foreach ($_POST['ad_del_users'] as $user_id) {
								$usr->delUserInGroup(url::getQueryAff(5), $user_id);
							}
						}
					} else if (url::getQueryAff(4) == 'add') {
						if (isset($_POST['ad_add_users'])) {
							foreach ($_POST['ad_add_users'] as $user_id) {
								$usr->addUserInGroup(url::getQueryAff(5), $user_id);
							}
						}
					}

					// Affichage des utilisateurs du groupe
					$tab = $usr->getUsersGroup(url::getQueryAff(5));
					if ($tab) {
						$size = sizeof($tab);
						for ($i = 0; $i < $size; $i++) {
							$tpl->set_var(array(
					...
					$tab_u = $usr->getUsersNotInGroup(url::getQueryAff(5));
					$size = sizeof($tab_u);
					for ($i = 0; $i < $size; $i++) {
						$tpl->set_var(array(
								'USER_ID'	=>	$tab_u[$i]->id,
					...
					$tpl->set_var(array(
							...
							'FORM_GROUP_EDIT_ADD'	=>	url::getPage(array('admin', 'groups', 'edit', 'add', url::getQueryAff(5))),
							'FORM_GROUP_EDIT_DEL'	=>	url::getPage(array('admin', 'groups', 'edit', 'del', url::getQueryAff(5))),
							));
		...
		if ($size) {
			for ($i = 0; $i < $size; $i++) {
				...
				$tpl->set_var(array(
						...
						'GROUP_ID'			=>	$tab[$i]->id,
						'GROUP_NAME'		=>	$tab[$i]->type == USR_TYPE_GRP ? '['.$tab[$i]->name.']' : $tab[$i]->name,
						'ADMIN_GROUP_EDIT'	=>	url::getPage(array('admin', 'groups', 'edit', 'view', $tab[$i]->id)),
						'ADMIN_GROUP_DEL'	=>	url::getPage(array('admin', 'groups', 'del', $tab[$i]->id)),
						));
		...
		$tpl->set_var(array(
				'ADMIN_GROUP_ADD'	=>	url::getPage(array('admin', 'groups', 'add')),
				));
	...
	case 'rights':
		...
		switch ($param) {
			...
			case 'add':
				...
				if (url::getQueryAff(4) == 'next' && $obj_dir && is_dir(FOLDER_ROOT.$obj_dir)) {
					...
					$tpl->set_var(array(
							...
							'FORM_RIGHTS_ADD'	=>	url::getPage(array('admin', 'rights', 'add', 'save'), $obj_dir),
							'OBJECT'			=>	$obj_dir,
							));
					...
					$tpl->set_var(array(
							'FORM_RIGHTS_ADD'	=>	url::getPage(array('admin', 'rights', 'add', 'next')),
							));
			...
			case 'edit':
				...
				if (url::getQueryAff(4) == 'save') {
					...
					list($obj_id, $user_id) = explode('|', url::getQueryAff(5));
				...
				list($obj_id, $user_id) = explode('|', url::getQueryAff(4));
				...
				if ($iuser && $ifile) {
					...
					$tpl->set_var('FORM_RIGHTS_EDIT', url::getPage(array('admin', 'rights', 'edit', 'save', $obj_id.'|'.$user_id)));
		...
		if (!$param) {
			...
			if ($t_error) {
				$tpl->set_var('URL_RIGHTS_REPAIR', url::getPage(array('admin', 'rights', 'repair')));
			...
			if ($tab) {
				...
				foreach ($tab as $occ) {
					...
					if ($last_obj != $occ['obj_id']) {
						$tpl->set_var(array(
								...
								'URL_OBJ'			=>	url::getObj($occ['obj_file']),
								'URL_RIGHTS_ADD'	=>	url::getPage(array('admin', 'rights', 'add', 'next'), $occ['obj_file']),
								));
					...
					$tpl->set_var(array(
							...
							'URL_RIGHTS_EDIT'	=>	url::getPage(array('admin', 'rights', 'edit', $occ['obj_id'].'|'.$occ['user_id'])),
				...
				$tpl->set_var('FORM_RIGHT_DEL', url::getPage(array('admin', 'rights', 'del')));
		...
		$tpl->set_var(array(
				'MSG'				=>	$msg,
				'URL_RIGHTS_ADD'	=>	url::getPage(array('admin', 'rights', 'add')),
				'URL_RIGHTS_LIST'	=>	url::getPage(array('admin', 'rights')),
				));
	...
	case 'comment':
		...
		if (url::getQueryAff(3) == 'del') {
			if (intval(url::getQueryAff(4)))
				$obj->delComment(intval(url::getQueryAff(4)));
			else {
		...
		$tpl->set_var(array(
				...
				'FORM_COMMENT_DEL'	=>	url::getPage(array('admin', 'comment', 'del')),
				));
	...
	case 'conf':
		...
		if (url::getQueryAff(3) == 'save') {
			...
			if (plugins::isValid($_POST['conf_dir_default_plugin']))
				$ini->editVar('dir_default_plugin', 	$_POST['conf_dir_default_plugin']);
			...
			if (!$ini->saveFile())
				...
				if ($tpl_changed) {
					redirect($cobj->file, url::getPage(array('admin', 'conf')), __('The new template will be applied !'));
					system::end();
				}
		...
		foreach ($tab_plugins as $occ) {
			$tpl->set_var(array(
					...
					'CONF_PLUGIN_NAME'		=>	(strtolower($occ['name']) == strtolower($conf['dir_default_plugin']) ? 'selected="selected"' : null)
					));
	...
	case 'anon':
		...
		switch (url::getQueryAff(3)) {
			#	Téléchargement
			case 'download':
				if (file::getRealFile(url::getQueryObj(), DIR_ROOT.DIR_ANON)) {
					file::sendFile(DIR_ROOT.DIR_ANON.url::getQueryObj());
					system::end();
				}
				break;
			...
			case 'accept':
				$ret = $lobj->acceptAnonFile(url::getQueryObj(), FOLDER_ROOT);
				if ($ret)
			...
			case 'del':
				$file = $lobj->getInfo(url::getQueryObj() ,false, false);
		...
		$tab = $lobj->getDirContent('/', null, 0, 10000, -1, array(array('=', 'type', TYPE_FILE), array('!', 'name', '.htaccess')));
		...
		if ($size) {
			for ($i = 0, $cmpt = 0; $i < $size; $i++) {
				...
				$tpl->set_var(array(
						...
						'ADMIN_ANON_MOVE'	=>	url::getPage(array('admin', 'anon', 'accept'), $tab[$i]->file),
						));
...
$tpl->set_var(array(
		...
		'ADMIN_PAGE_GROUPS'			=>	url::getPage(array('admin', 'groups')),
		'ADMIN_PAGE_RIGHTS'			=>	url::getPage(array('admin', 'rights')),
